...
<div id='printMe'>
<div class="container"> 
<h1>Riwayat Lelang Kamu</h1>
<!-- <form method="POST" action="tampil_lelang.php" class="d-flex center">
  <input class="form-control me-2" type="search" name="cari" placeholder="Search" aria-label="Search">
  <button class="btn btn-outline-success" type="submit">Search</button>
</form> -->
<br> 
<form action= "proses_history.php" method="post" enctype="multipart/form-data" >
                <table class="table table-bordered border-primary">
                    ...
                    <tbody>
                        ...
                        $query_barang = mysqli_query($koneksi, "SELECT * FROM barang where status = 'tutup'");
                        $data_barang = mysqli_fetch_array($query_barang);
                        $no=0;
                        while($data_lelang=mysqli_fetch_array($query_lelang)){
                            $no++;
                        ?>
                    <form action= "proses_history.php" method="POST">
                        <tr>
                            <td><?=$no?></td>
