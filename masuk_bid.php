        ...
        where id_barang='".$_GET['id_barang']."'");
            ...
            if ($query_harting) {
                ...
                if ($query_beli) {
                    ...
                    echo "<script>alert('Selamat anda penawar tertinggi');location.href='lelang.php'</script>";
                }
                else{
                    echo "<script>alert('Gagal tertinggi');location.href='lelang.php'</script>";
                }
            }
            else{
                echo "<script>alert('Gagal lelang');location.href='lelang.php'</script>";
            }
        }else{
            echo "<script>alert('penawaran harus lebih dari harga tertinggi');location.href='lelang.php'</script>";
        }
    }else{
        echo "<script>alert('penawaran harus lebih dari harga buka lelang');location.href='lelang.php'</script>";
        // $query_beli = mysqli_query($conn, "INSERT INTO tb_lelang (id_user, id_barang, id_petugas, tgl_lelang)
        //     VALUES ('".$_SESSION['id_user']."','".$_GET['id_barang']."', '2','".date('Y-m-d')."')");
